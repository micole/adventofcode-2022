...
Move groups
'''
def move_items(listfrom, listto, number):
    new_listfrom = []
    new_listto = []

    #new_listto.append(listfrom[:number-1])
    #new_listto.append(listto)
    new_listto = listfrom[:number] + listto
    new_listfrom = listfrom[number:]

    return(new_listfrom, new_listto)
...
for index, line in enumerate(lines):
    if line.startswith("move"):
        ...
        print(line)
        clean_list[int(commands[3]) - 1], clean_list[int(commands[5]) - 1] = move_items(
            clean_list[int(commands[3]) - 1], clean_list[int(commands[5]) - 1], int(commands[1]))
